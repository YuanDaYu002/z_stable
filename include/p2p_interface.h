...
typedef struct _p2p_sync_value_t {
    char name[128];
    char value[128];
} p2p_sync_value_t;
...
#endif
    int data_type;	// 0 是矩形，1 是八边形
    p2p_point_t octagon[8];
...
typedef struct _p2p_push_condition_t {
    ...
    int  if_push;           // 是否要推送
    int  picture_catch_interval; // 次告警时间间隔，单位秒
    char movition_last[16];     // 运动持续时间，单位秒
    int  is_main;               // 是否是主要告警， 1为是，0为不是
    char alarm_interval_span[16]; // 两次有效告警触发的时间间隔，为区间形式，单位秒
    int  move_length;           //目标的移动距离
} p2p_push_condition_t;

typedef struct _p2p_alarm_stream_level {
    int bit_rate;   //码率大小
    int frame_rate; //帧率
    int delay;  //主告警后等待多久执行此码流等级
} p2p_alarm_stream_level;
...
typedef struct
{
	...
	*/
	...
	/* 915模块标示，wor门铃上报此字段发送给中继 */
	const char *module_id_915;
	/*
	设置客户端接听设备来电事件的回调函数, 
	...
	*/
	...
	/*
		使设备进入ap 模式的回调函数
		@mode  1：表示进入软AP模式; 2:表示进入添加被动设备的监听模式 3: 表示开门指令
		@device_count 要连接的设备数量
        @extra_para 额外参数
	*/
	void (*on_ap_mode_callback)(int mode, int device_count, const char* extra_para);
    ...
    /*
     * 主报警上报失败, 系统层需要把下一个次告警当成主告警
     * @result 1 成功， 0 失败
     * @is_main 是否主告警
     */
    void (*on_alert_report_result)(int result, int is_main, int alert_type, int trigger_time);
     ...
     * @sl_num 码率等级个数
     */
    void (*on_set_pir_alarm_strategy)(int interval, int video_last, int s_time, int cloud_m_video_last, int cloud_s_video_last, p2p_alarm_stream_level* sl, int sl_size);

    /*
     * @sl 码率等级信息
     * @sl_num 码率等级个数
     */
    void (*on_set_md_alarm_strategy)(int cloud_m_video_last, int cloud_s_video_last, p2p_alarm_stream_level* sl, int sl_size);
     ...
     */
    void (*on_set_stream_temporary)(int type, int stream_type, int frame_rate, int bit_rate);
     ...
     * return 0表示成功，否则失败
     * */
    int (*on_unlocking)(int op_type, const char* physical_id);
	/**
	 * 临时设置或者恢复码流的gop
	 * @type 1为临时设置，0为恢复默认
	 * @gop I帧间隔,1~10S	 google 默认设置成1 S
	 */
	void (*on_set_stream_gop)(int type, int gop);
}p2p_init_info_t;
...
/*
 * 批量上传值
 * @values_count  p2p_sync_value_t的个数
 * */
...
typedef struct _p2p_face_pos_t {
    int x;
    int y;
    int w;
    int h;
...
typedef void(*config_key_cb_t)(const char* key, const char* value);
/*
 * 注册一个回调函数， 当服务器下发某个字段时调用此函数
 *
 */
