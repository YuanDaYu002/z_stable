...
public: 
	static CNetServer* m_instance ;
	static CNetServer* getInstance(){
		if( NULL == m_instance ){
			m_instance = new CNetServer() ;
		}
		return m_instance ;
	};
	static void release(){
...
protected:
	// tcp工作会话管理
	STRUCT_LOGIN_USER			m_loginUserList[32] ;
	CMutex						m_loginUserListMutex ;
	
	//报警会话
	STRUCT_ALARM_USER			m_alarmUserList[16] ;
